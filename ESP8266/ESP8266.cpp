...
ESP8266::ESP8266(SoftwareSerial &uart, uint32_t baud, boolean debug_mode): m_puart(&uart)
{
    baud_rate = baud;
    debug = debug_mode;
    //Serial.println(“Start with software serial”);
    //m_puart->begin(baud);
    //rx_empty();
}
#else
ESP8266::ESP8266(HardwareSerial &uart, uint32_t baud, boolean debug mode): m_puart(&uart)
{
    baud_rate = baud;
    debug = debug_mode;
    // removed by Uz
...
// ====== added by Uz
void ESP8266::begin_serial(void)
{
   //m_puart->end();
...
void ESP8266::set_baud(uint32_t baud)
{
   sATCIOBAUD(baud);
}

void ESP8266::is_joinAP(void)
{
    rx_empty();
...
bool ESP8266::enable_Watchdog(void)
{
	rx_empty();
    m_puart->println("AT+CSYSWDTENABLE");
    return recvFind("OK");
}

bool ESP8266::disable_Watchdog(void)
{
	rx_empty();
    m_puart->println("AT+CSYSWDTDISABLE");
    return recvFind("OK");
}


bool ESP8266::ping(void)
{
	rx_empty();
	m_puart->println("AT+PING=\"192.168.1.1\"");
	return recvFind("OK");
}
// ==== end of Uz’s code
